...
- **Probes (Liveness/Readiness/Startup):** Probes check container health. Kubernetes supports three probe types.  
  - `livenessProbe`: if it fails, the kubelet kills and restarts the container.  
  - `readinessProbe`: if it fails, the Pod is removed from Service endpoints (it stops receiving traffic).  
  - `startupProbe`: used for slow-startup apps; other probes are disabled until it succeeds.  
  ...
  **Probe Comparison Table:**
  ...
  | -------------- | ------------------------------------------- | --------------------------------------------- |
  | livenessProbe  | Checks if container is running/healthy.     | Failure kills container; restartPolicy applies. |
  | readinessProbe | Checks if container is ready to serve.      | Failure removes Pod IP from Service endpoints (no restart). |
  | startupProbe   | Checks if container startup is complete.    | Disables other probes until success; failure kills container (like liveness). |

- **Resources (requests/limits):** Containers can specify `resources.requests` and `resources.limits` for CPU/memory. Requests influence scheduling; limits are enforced (CPU throttling, OOM kill). Example:
  ```yaml
  spec:
    containers:
    - name: busy
...
- **Environment Variables:** Use `env` or `envFrom` to inject variables. Examples:
  ...
  ```
  Kubernetes also supports `envFrom: configMapRef` or `secretRef` to load multiple values. The Downward API can expose Pod metadata (like namespace, labels) as env vars or files. Apply and view: `kubectl logs env-demo-pod`.

- **ConfigMap/Secret Volumes:** You can mount a ConfigMap or Secret as a volume in a Pod. Example:
  ```yaml
  apiVersion: v1
  kind: ConfigMap
    ...
    - name: cfg
      configMap:
        ...
        - key: log_level
          path: log_level.conf
  ```
  The ConfigMap’s `log_level` data is mounted at `/etc/config/log_level.conf`. Apply both: `kubectl apply -f cm-pod.yaml`. Inspect with `kubectl describe pod cm-pod`. Cleanup: `kubectl delete pod cm-pod` and `kubectl delete configmap log-config`.
...
- **Ephemeral Containers:** Kubernetes supports **ephemeral containers** (v1.25+) for debugging existing Pods. These are not specified in the Pod YAML; you add them at runtime (e.g. via `kubectl debug`). They do not restart automatically and have limited fields. Example flow:
  ...
  Ephemeral containers are useful for troubleshooting dead pods.

- **Topology Spread Constraints:** `spec.topologySpreadConstraints` can spread Pods across zones or nodes for availability. For example:
  ```yaml
  spec:
...
Kubernetes volumes (ephemeral or persistent) allow data sharing and persistence. Volumes are declared under `spec.volumes` and then mounted into containers via `volumeMounts`. Key volume types:

- **emptyDir (Ephemeral):** An `emptyDir` volume is initially empty and lasts as long as the Pod does. All containers in the Pod can read/write it, and it is erased when the Pod is removed. Example (scratch space):
  ```yaml
  volumes:
    - name: scratch
      emptyDir: {}
...
- **hostPath (Node-specific, Ephemeral):** A `hostPath` volume mounts a file or directory from the node’s filesystem into the Pod. For example:
  ```yaml
  volumes:
    - name: host-vol
      hostPath:
  ...
  containers:
    - name: hostpath-demo
      ...
      - name: host-vol
        mountPath: /mnt
  ```
  This shares the host’s `/tmp/host-data` with the container. **Warning:** hostPath bypasses isolation and can be a security risk. After testing, delete Pod with `kubectl delete pod`.

- **configMap (Ephemeral):** Shown above. A ConfigMap mounted as a volume is read-only and contains the data from the ConfigMap. Similar for **Secret** volumes: secret data as files. Example:
  ```yaml
  apiVersion: v1
  kind: Secret
...
- **PersistentVolume (PV) & PersistentVolumeClaim (PVC):** PersistentVolumes are cluster resources representing durable storage. On Minikube, the default provisioner uses `hostPath` behind the scenes. A PVC is a request for storage. You can either create a static PV or rely on dynamic provisioning with a StorageClass.
  ...
  **Static PV Example:**  
  ...
  ```
  This PV uses hostPath on the Minikube VM (Minikube preserves `/data` by default). Create it: `kubectl apply -f pv-local.yaml`.
  ...
  **PVC Example:**  
  ...
  ```
  After `kubectl apply -f pvc-local.yaml`, the PVC will bind to the available PV (`pv-local`) based on matching size and access mode. Check: `kubectl get pv,pvc`.

  **Dynamic Provisioning:** Minikube includes a dynamic hostPath provisioner. You can use the default StorageClass (likely `standard` or none) by omitting `storageClassName` or specifying it. Example:
  ```bash
  ...
  **Access Modes and Reclaim Policies:**  
  - AccessModes: 
    - `ReadWriteOnce` (RWO): mount RW by a single node (common for hostPath/local PV).  
  ...
  - reclaimPolicy: what happens when PVC is deleted. Options: `Retain`, `Delete`, `Recycle`. For dynamically provisioned PVs, the StorageClass default reclaim policy is usually `Delete` (meaning delete the underlying storage).  
  ...
  **Snapshots:** Minikube supports volume snapshots if you enable the `csi-hostpath-driver` and `volumesnapshots` addons. After enabling, you can `VolumeSnapshotClass` and create `VolumeSnapshot` objects. (Detailed steps are in [the CSI HostPath tutorial]; not repeated here.)

  **Comparison of Volume Types:** Below is a summary of common volume types:

  | Volume Type         | Ephemeral/Persistent | Description                                            |
  | ------------------- | -------------------- | ------------------------------------------------------ |
  | **emptyDir**        | Ephemeral (pod-local) | Created empty on pod start; shared between containers; deleted with pod. |
  | **hostPath**        | Ephemeral/Persistent | Mounts a host file/directory into pod. (Not portable; local to node.) |
  | **ConfigMap/Secret**| Ephemeral (pod-local) | Injects ConfigMap or Secret as files (read-only). |
  | **PersistentVolume**| Persistent           | Abstracts durable storage (NFS, hostPath, cloud disks); exists beyond pod life. |
  | **CSI volume**      | Persistent           | Uses CSI drivers for external storage (e.g. CSI HostPath). Supports snapshots/addons. |
  ...
  **Lifecycle:** When a PVC is created, Kubernetes control-plane binds it to a matching PV (static or dynamic). The flow is: PVC request → (if StorageClass specified) dynamic provisioning → PV created → PVC bound → Pod using PVC → Pod terminates (data stays on PV). A *Reclaim Policy* on PV (Retain/Delete) determines cleanup.  

  **Cleanup:** Delete any PVC/PV and Pods when done (e.g. `kubectl delete pod,pvc,pv myapp`). By default, a dynamically provisioned PV will be deleted with its PVC (if StorageClass is default-delete). Static PVs (Retain by default) must be manually cleaned if desired.
...
**Intra-Pod Networking:** All containers in a Pod share the same IP and port space. For example, a Pod with two containers can have one container listen on a port and the other access it via `localhost`. Example:
```yaml
apiVersion: v1
kind: Pod
...
- **NodePort:** Exposes Service on a port (usually 30000-32767) on each node’s IP. In Minikube:  
  ...
  ```
  Or simply use `minikube service hello --url` to get a URL, which sets up a local tunnel.  
...
**DNS:** Kubernetes’ DNS (CoreDNS) automatically creates DNS entries for Services and Pods. A Pod’s `/etc/resolv.conf` search path includes `<namespace>.svc.cluster.local`. Thus a Pod can resolve `myservice` or `myservice.namespace` to the ClusterIP. For example, in the client above, `nslookup server-svc` returns the service IP.
...
spec:
  ...
  policyTypes: ["Ingress"]
```
This policy matches all pods but has no `ingress` rules, so it blocks all incoming traffic to those pods. *Note:* Minikube’s default CNI (`kindnet`) does **not** enforce NetworkPolicies. To use NetworkPolicy, install a CNI that supports it (e.g. `minikube addons enable calico`). If enabled, verify with `kubectl apply -f policy.yaml` and test connectivity (blocked/allowed) between pods. (Refer to Kubernetes docs for policy details.)
...
**Ingress:** An Ingress defines external HTTP(S) access rules. Minikube has an NGINX Ingress controller addon. Example workflow (from Kubernetes ingress tutorial):  

...
graph TD
    ...
    subgraph ServiceDef [Service Definition]
        S[Service Selector]
        Req[Must have: <br/> app=web <br/> AND <br/> env=prod]
    end

    %% 2. The Filter
    S --> Logic{{"Selector Logic <br/> (AND Operation)"}}

    %% 3. The Candidates & Results
    subgraph Cluster [Candidate Pods]
        
        %% Case 1: Missing Label
        P1[Pod 1 <br/> labels: app=web] 
        Logic -.->|Missing 'env'| Block1[Traffic Blocked]
        Block1 --- P1
        
        %% Case 2: Wrong Value
        P2[Pod 2 <br/> labels: app=web, env=dev]
        Logic -.->|Mismatch 'env'| Block2[Traffic Blocked]
        Block2 --- P2
        
        %% Case 3: Exact Match
        P3[Pod 3 <br/> labels: app=web, env=prod]
        Logic ==>|Match| Allow1[Traffic Allowed]
        Allow1 ==> P3

        %% Case 4: Superset Match
        P4[Pod 4 <br/> labels: app=web, env=prod, region=us]
        Logic ==>|Match| Allow2[Traffic Allowed]
        Allow2 ==> P4
    end
```
---
...
flowchart TD
    ...
    subgraph ClusterScope [Kubernetes Cluster]
        
        subgraph WorkerNodes [Worker Nodes]
            N["<b>NodePort</b>"]
            
            subgraph InternalNetwork [Cluster Internal Network]

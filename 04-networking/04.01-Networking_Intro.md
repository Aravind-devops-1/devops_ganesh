...
Since Pods are ephemeral (they die and get replaced with new IP addresses), the Service provides a **single, persistent DNS name and IP address** (ClusterIP) to act as a stable entry point, automatically load-balancing traffic across all matching Pods.
